...
from olclient.helper_classes.author_results import AuthorResults
...
class OpenLibrary:
    ...
    def Author(ol_self):
        class Author(common.Author):
            ...
            @classmethod
            def q(cls, q: str, offset: int | None = None, limit: int | None = None) -> List[common.Author]:
                url = f'{cls.OL.base_url}/search/authors.json?q={q}'
                if offset:
                    url += f'&offset={offset}'
                if limit:
                    url += f'&limit={limit}'
                ...
                @backoff.on_exception(
                    on_giveup=lambda error: logger.exception(
                        "Error retrieving metadata for author: %s", error
                    ),
                    **cls.OL.BACKOFF_KWARGS,
                )
                def _get_authors_by_metadata(ol_url):
                    return cls.OL.session.get(ol_url)

                response = _get_authors_by_metadata(url)
                results = AuthorResults(**response.json())

                authors = []
                for a in results.docs:
                    try:
                        author = a.to_author()
                        c = cls.get(author.olid)
                        if author.bio == "":
                            author.bio = c.bio
